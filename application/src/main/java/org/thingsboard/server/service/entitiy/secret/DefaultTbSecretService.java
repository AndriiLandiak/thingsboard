...
public class DefaultTbSecretService extends AbstractTbEntityService implements TbSecretService {
    ...
    @Override
    public Secret save(Secret secret, User user) throws ThingsboardException {
        ActionType actionType = secret.getId() == null ? ActionType.ADDED : ActionType.UPDATED;
        TenantId tenantId = secret.getTenantId();
        try {
            Secret savedSecret = checkNotNull(secretService.saveSecret(tenantId, secret));
    ...
    public void delete(Secret secret, User user) {
        ...
        TenantId tenantId = secret.getTenantId();
        SecretId secretId = secret.getId();
        try {
            secretService.deleteSecret(tenantId, secretId);
            logEntityActionService.logEntityAction(tenantId, secretId, secret, actionType, user, secretId.toString());
        } catch (Exception e) {
