...
public class SecretDataValidator extends DataValidator<Secret> {
    ...
    protected void validateDataImpl(TenantId tenantId, Secret secret) {
        if (secret.getTenantId() == null) {
            throw new DataValidationException("Secret should be assigned to tenant");
        }
        if (StringUtils.isEmpty(secret.getName())) {
            throw new DataValidationException("Secret name should be specified!");
        }
    }
